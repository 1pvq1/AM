...
fun listFileViaJavaIO(path: String, maxDepth: Int = Int.MAX_VALUE): List<FileNode> {
    val root = File(path)
    if (!root.exists() || !root.isDirectory) return emptyList()

    fun traverse(file: File, currentDepth: Int): FileNode {
 ...
 */
@OptIn(ExperimentalTime::class)
fun listFileViaKotlinIO(path: String, maxDepth: Int = Int.MAX_VALUE): List<FileNode> {
    val root = File(path)
    if (!root.exists() || !root.isDirectory) return emptyList()

    val nodeCache = mutableMapOf<String, FileNode>()
